...
let funded = GAMES_JSON.filter((game) => game.pledged >= game.goal).length;
let unfunded = GAMES_JSON.filter((game) => game.pledged < game.goal).length;
let totalPledged = GAMES_JSON.reduce((acc, game) => acc + game.pledged, 0);

// create a string that explains the number of unfunded games using the ternary operator
let statement = `So far, $${totalPledged.toLocaleString()} has been raised for ${
  GAMES_JSON.length
} games. `;
statement += `Out of these, ${funded} ${
  funded === 1 ? "game has" : "games have"
} been fully funded and `;
statement += `${unfunded} ${
  unfunded === 1 ? "game remains" : "games remain"
} unfunded.`;

// create a new DOM element containing the template string and append it to the description container
let desc = document.createElement("p");
desc.innerHTML = statement;
